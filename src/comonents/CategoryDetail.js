...
const CategoryDetail = () => {
  ...
  const deleteData = async () => {
    try {
      const res = await axios.delete(`${process.env.REACT_APP_BASE_URL}/categorys/delete/${id}`)
      console.log('RES=>', res);
  ...
  const getCategoryName = async () => {
    try {
      const res = await axios.get(`${process.env.REACT_APP_BASE_URL}/categorys/${id}`);
      console.log("res=>", res);
  ...
  const handleSubmit = async (e) => {
    try {
      e.preventDefault()
      const resp = await axios.put(`${process.env.REACT_APP_BASE_URL}/categorys/update/${id}`, { title })
