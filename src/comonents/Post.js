...
const Post = () => {
  ...
  const fetchPost = async () => {
    try {
      const resp = await axios.get(`${process.env.REACT_APP_BASE_URL}/products`);
      // console.log(resp.data.data);
      const productSortByDate = resp.data.data.sort((a, b) => {
        const dateA = new Date(a.createdAt)
        const dateB = new Date(b.createdAt)
        return dateB - dateA
      })

      setPost(productSortByDate);
    } catch (error) {
      console.log(error);
    }
  };
        ...
        <div className="container">
          <div className="row">
            <div className="col-md-6">
              <h1>
                <i className="fas fa-pencil-alt"></i> Products
              </h1>
            </div>
          </div>
        </div>
                ...
                <table className="table table-striped">
                  ...
                  <tbody>
                    {records.filter((val) => val.pName.toLowerCase().includes(query) ||
                      val.categoryId.category.toLowerCase().includes(query)
                    ).map((item, index) => {
