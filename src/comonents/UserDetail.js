...
// update user 
  ...
  const handleSubmit = async (e) => {
    try {
      e.preventDefault()
      const resp = await axios.put(`${process.env.REACT_APP_BASE_URL}/users/update/${id}`,{name,email});
...
// fetch user by Id
  ...
  const fetchUser = async () => {
    try {
      const resp = await axios.get(`${process.env.REACT_APP_BASE_URL}/users/${id}`);
  ...
  const deleteUser = async() =>{
    try {
      const resp = await axios.delete(`${process.env.REACT_APP_BASE_URL}/users/delete/${id}`)
      console.log(resp)
